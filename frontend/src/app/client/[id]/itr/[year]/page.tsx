...
export default function ItrFormPage() {
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await itrApi.downloadExcel(clientId, year);
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `ITR1_Report_${year}.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
      setSuccessMsg('Excel report downloaded');
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err: any) {
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await itrApi.downloadPdf(clientId, year);
      const blob = new Blob([res.data], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `ITR1_${clientId}_${year}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      setSuccessMsg('PDF report downloaded');
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err: any) {
      // responseType:'blob' means error body is also a blob — read it as text
      if (err.response?.data instanceof Blob) {
        const text = await err.response.data.text();
        try {
          const json = JSON.parse(text);
          setError(json.message || 'PDF generation failed');
        } catch {
          setError(text || 'PDF generation failed');
        }
      } else {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8', fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
      ...
      <header style={{ background: '#0d2137', borderBottom: '1px solid #1a3550', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 16px', height: 46, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 26, height: 26, borderRadius: 6, background: 'linear-gradient(135deg,#1976d2,#42a5f5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: '#fff' }}>₹</div>
            <span style={{ fontSize: 13, fontWeight: 800, color: '#e2eaf6' }}>MyTaxERP</span>
          </div>
          <span style={{ color: '#1a3550', fontSize: 15 }}>›</span>
          <button onClick={() => router.back()} style={{ background: 'none', border: 'none', color: '#7a9ab8', cursor: 'pointer', fontSize: 12 }}>Dashboard</button>
          <span style={{ color: '#1a3550', fontSize: 15 }}>›</span>
          <div>
            <span style={{ fontSize: 12.5, fontWeight: 700, color: '#e2eaf6' }}>ITR-1 Form — {year}</span>
            <span style={{ fontSize: 11, color: '#5a7a9a', marginLeft: 10 }}>
              {form.partA?.assesseeName} &middot; {form.partA?.pan} &middot;{' '}
              <span style={{ color: isNew ? '#60a5fa' : '#fb923c', fontWeight: 600 }}>{isNew ? 'New Regime' : 'Old Regime'}</span>
            </span>
          </div>
          <div style={{ flex: 1 }} />
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <button onClick={handleSave} disabled={saving} style={{ padding: '5px 12px', background: '#1a3550', border: '1px solid #2a4f70', borderRadius: 5, color: '#c8daea', cursor: 'pointer', fontSize: 11.5, fontWeight: 600 }}>
              {saving ? 'Saving…' : 'Save'}
            </button>
            <button onClick={handleCompute} disabled={computing} style={{ padding: '5px 12px', background: '#1565c0', border: 'none', borderRadius: 5, color: '#fff', cursor: 'pointer', fontSize: 11.5, fontWeight: 700 }}>
              {computing ? 'Computing…' : 'Compute Tax'}
            </button>
            <button onClick={handleDownloadPdf} style={{ padding: '5px 12px', background: '#dc2626', border: 'none', borderRadius: 5, color: '#fff', cursor: 'pointer', fontSize: 11.5, fontWeight: 600 }}>PDF</button>
            <button onClick={handleDownloadExcel} style={{ padding: '5px 12px', background: '#059669', border: 'none', borderRadius: 5, color: '#fff', cursor: 'pointer', fontSize: 11.5, fontWeight: 600 }}>Excel</button>
            <button onClick={handleDownload} style={{ padding: '5px 12px', background: '#16a34a', border: 'none', borderRadius: 5, color: '#fff', cursor: 'pointer', fontSize: 11.5, fontWeight: 600 }}>JSON</button>
          </div>
        </div>
      </header>
